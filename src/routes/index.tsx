...
import {
  Plane, Train, MapPin, Mountain, Briefcase, Camera, Users, Hotel,
  Heart, UsersRound, Clock, ShieldCheck, Sparkles, Wallet, BadgeCheck,
  Star, Phone, Mail, MessageCircle, ArrowRight, Car,
} from "lucide-react";

import { Navbar } from "@/components/Navbar";
import { Footer } from "@/components/Footer";
import { useBooking } from "@/components/BookingProvider";
import { buildBookingMessage, waLink, WA_NUMBER, PHONE_DISPLAY, EMAIL, ADDRESS } from "@/lib/whatsapp";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Reliable taxi service in Dibrugarh: airport transfers, outstation cabs, Innova rental, and Assam & Arunachal tour packages. Book on WhatsApp 24×7." },
      { name: "keywords", content: "Taxi Service Dibrugarh, Cab Service Dibrugarh, Airport Taxi Dibrugarh, Assam Taxi Service, Arunachal Taxi Service, Tour Packages Assam, Innova Rental Dibrugarh, Cab Booking Assam, Travel Agency Dibrugarh" },
      { property: "og:title", content: "Dibrugarh Go Cabs — Trusted Taxi Service Across Assam" },
      { property: "og:description", content: "Airport transfers, outstation cabs and tour packages across Assam & Arunachal. Book instantly on WhatsApp." },
      ...
      <div className="mx-auto grid max-w-7xl gap-10 px-4 pt-14 pb-20 sm:px-6 lg:grid-cols-[1.05fr_1fr] lg:gap-14 lg:pt-20 lg:pb-28">
        <div className="text-background">
          ...
          <p className="mt-5 max-w-xl text-base text-background/80 sm:text-lg">
            Airport transfers, local taxis, outstation trips, tourist packages, corporate travel and round-the-clock cab service — handled by professional drivers and spotless vehicles.
          </p>

          <div className="mt-7 flex flex-wrap gap-3">
            <BookCTA size="lg" label="Book Your Cab" />
            <a
              href={waLink("Hi Dibrugarh Go Cabs 👋\n\nI'd like an instant fare quote. Please share details.")}
              target="_blank"
              rel="noreferrer"
          ...
          <div className="mt-10 flex flex-wrap items-center gap-x-6 gap-y-3 text-xs text-background/70">
            <span className="flex items-center gap-2"><BadgeCheck className="h-4 w-4 text-gold" /> Verified Drivers</span>
            <span className="flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-gold" /> Safe & Insured</span>
            <span className="flex items-center gap-2"><Wallet className="h-4 w-4 text-gold" /> Transparent Pricing</span>
          </div>
        </div>
...
function HeroBookingCard() {
  const [form, setForm] = useState({
    pickup: "", destination: "", journeyType: "One Way",
    date: "", time: "", passengers: "", name: "", phone: "", requirement: "",
  });
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-3">
        <Inp label="Pickup *" required value={form.pickup} onChange={(v) => setForm({ ...form, pickup: v })} placeholder="Dibrugarh Airport" />
        <Inp label="Destination *" required value={form.destination} onChange={(v) => setForm({ ...form, destination: v })} placeholder="Tinsukia" />
        ...
        <div className="col-span-2">
          <Inp label="Phone *" required type="tel" value={form.phone} onChange={(v) => setForm({ ...form, phone: v })} placeholder="+91 XXXXXXXXXX" />
        </div>
        <div className="col-span-2">
          <Inp label="Special Requirement" value={form.requirement} onChange={(v) => setForm({ ...form, requirement: v })} placeholder="Need Innova Crysta" />
        </div>
      </div>
...
function Inp(props: { label: string; required?: boolean; type?: string; value: string; onChange: (v: string) => void; placeholder?: string }) {
  return (
...
function Services() {
  const services = [
    { icon: Plane, title: "Airport Transfers", desc: "On-time pickups & drops from Dibrugarh Airport." },
    { icon: Train, title: "Railway Station Pickup", desc: "Comfortable rides to and from the station." },
    { icon: MapPin, title: "Local Taxi Service", desc: "Hourly & full-day local cabs across Dibrugarh." },
    { icon: Mountain, title: "Outstation Cab Service", desc: "One-way & round trips across Assam and beyond." },
    { icon: Briefcase, title: "Corporate Travel", desc: "Reliable transport for teams and executives." },
    { icon: Camera, title: "Tourist Taxi Service", desc: "Curated sightseeing with knowledgeable drivers." },
    ...
    { icon: Sparkles, title: "Wedding Transportation", desc: "Decorated cabs and fleet for your big day." },
    ...
    { icon: Car, title: "Custom Itineraries", desc: "Tell us your plan — we'll route the perfect ride." },
  ];

  return (
    <Section id="services" eyebrow="Our Services" title="Travel-grade rides for every kind of journey">
...
function ServiceCard({ icon: Icon, title, desc }: { icon: typeof Plane; title: string; desc: string }) {
...
function Tours() {
  ...
  return (
    <Section id="tours" eyebrow="Tour Packages" title="Curated journeys across Northeast India" dark>
...
function TourCard({ name, img, days, tag }: { name: string; img: string; days: string; tag: string }) {
  const { open } = useBooking();
  return (
    <article className="group overflow-hidden rounded-3xl border border-background/10 bg-ink shadow-card">
      <div className="relative aspect-[4/3] overflow-hidden">
        <img src={img} alt={name} loading="lazy" className="h-full w-full object-cover transition duration-700 group-hover:scale-110" />
        <div className="absolute inset-0 bg-gradient-to-t from-ink via-ink/40 to-transparent" />
        <span className="absolute left-4 top-4 rounded-full bg-gradient-fire px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-ink">{tag}</span>
        <span className="absolute right-4 top-4 rounded-full bg-background/15 px-3 py-1 text-[10px] font-semibold uppercase tracking-wider text-background backdrop-blur">{days}</span>
        <h3 className="absolute bottom-4 left-4 right-4 font-display text-xl font-bold text-background">{name}</h3>
      </div>
      <div className="grid grid-cols-3 gap-2 p-4">
        <button
          onClick={() => open({ subject: `Tour: ${name}`, title: "Request Quote", defaults: { destination: name } })}
        ...
        <button
          onClick={() => open({ subject: `Tour Quote: ${name}`, title: "Request Quote", defaults: { destination: name, journeyType: "Round Trip" } })}
        ...
        <button
          onClick={() => open({ subject: `Book Tour: ${name}`, title: "Book This Tour", defaults: { destination: name, journeyType: "Round Trip" } })}
  ...
  return (
    <Section id="fleet" eyebrow="Our Fleet" title="A spotless, well-maintained line-up">
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {fleet.map((v, i) => <FleetCard key={v.name} {...v} img={[fleetAnini.url, fleetYard.url, fleetMountains.url][i % 3]} />)}
      </div>
    </Section>
  );
}

function FleetCard({ name, seats, desc, img }: { name: string; seats: string; desc: string; img: string }) {
  const { open } = useBooking();
  return (
    <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-card transition hover:-translate-y-1">
      ...
      <div className="p-5">
        ...
        <div className="mt-4 grid grid-cols-2 gap-2">
          <button
            onClick={() => open({ subject: `Vehicle Enquiry: ${name}`, title: "Get Fare", defaults: { requirement: `Need ${name}` } })}
            className="rounded-lg border border-border px-3 py-2 text-xs font-semibold text-foreground hover:border-ember/50"
          >
            Get Fare
          </button>
          <button
            onClick={() => open({ subject: `Book Vehicle: ${name}`, title: "Book Now", defaults: { requirement: `Need ${name}` } })}
            className="rounded-lg bg-gradient-fire px-3 py-2 text-xs font-bold text-ink shadow-glow"
          >
            Book Now
          </button>
    ...
    <Section id="why" eyebrow="Why Choose Us" title="A travel partner you can count on">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {items.map(([Icon, t, d]) => {
          ...
          return (
            <div key={t as string} className="flex gap-4 rounded-2xl border border-border bg-card p-5 shadow-card">
...
function Gallery() {
  const imgs = [
    { src: fleetAnini.url, cat: "Arunachal Tours", alt: "Fleet of Innovas in Anini, Arunachal Pradesh" },
    ...
    <Section id="gallery" eyebrow="Gallery" title="On the road with Dibrugarh Go Cabs">
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
        {imgs.map((im, i) => (
          ...
          >
            <img src={im.src} alt={im.alt} loading="lazy" className="h-full w-full object-cover transition duration-700 group-hover:scale-110" />
      ...
      {active !== null && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-ink/90 p-4" onClick={() => setActive(null)}>
          <img src={imgs[active].src} alt={imgs[active].alt} className="max-h-[90vh] max-w-full rounded-2xl object-contain shadow-glow" />
        </div>
      )}
    </Section>
  );
  ...
  return (
    <Section eyebrow="Testimonials" title="Loved by travellers across the Northeast" dark>
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
        {reviews.map(([q, n]) => (
          <figure key={n} className="rounded-2xl border border-background/10 bg-background/5 p-6 backdrop-blur">
            <div className="flex gap-0.5 text-gold">
              {Array.from({ length: 5 }).map((_, i) => <Star key={i} className="h-4 w-4 fill-current" />)}
            </div>
            <blockquote className="mt-3 text-sm text-background/90">"{q}"</blockquote>
            <figcaption className="mt-4 text-xs font-semibold uppercase tracking-wider text-gold">— {n}</figcaption>
          </figure>
        ))}
      </div>
    </Section>
          ...
          <div className="relative grid gap-6 sm:grid-cols-[1.4fr_1fr] sm:items-center">
            <div>
              ...
              <p className="mt-3 max-w-xl text-background/80">
                Get instant fare details and booking confirmation directly on WhatsApp. We're available 24×7 across Assam and Arunachal Pradesh.
              </p>
            </div>
...
function Contact() {
  const [form, setForm] = useState({ name: "", phone: "", pickup: "", destination: "", date: "", message: "" });
  function submit(e: React.FormEvent) {
    e.preventDefault();
    const msg = buildBookingMessage({
      name: form.name, phone: form.phone,
      pickup: form.pickup, destination: form.destination, date: form.date,
    ...
    <Section id="contact" eyebrow="Contact Us" title="Plan your next ride with us">
      <div className="grid gap-8 lg:grid-cols-[1fr_1.2fr]">
          ...
          {[
            ...
            const Body = (
              <div className="flex items-start gap-4 rounded-2xl border border-border bg-card p-5 shadow-card">
                ...
                <div>
                  <p className="text-xs font-bold uppercase tracking-widest text-ember">{label as string}</p>
                  <p className="mt-1 text-sm font-medium text-foreground">{val as string}</p>
                </div>
              </div>
            );
            return href ? <a key={label as string} href={href as string} target={(href as string).startsWith("http") ? "_blank" : undefined} rel="noreferrer">{Body}</a> : <div key={label as string}>{Body}</div>;
          })}
        </div>
        ...
        <form onSubmit={submit} className="rounded-3xl border border-border bg-card p-6 shadow-card sm:p-8">
          ...
          <button type="submit" className="mt-5 w-full rounded-xl bg-gradient-fire px-6 py-4 text-base font-bold text-ink shadow-glow transition hover:scale-[1.01]">
            Send via WhatsApp
          </button>
        </form>
      </div>
    ...
    <section id={id} className={dark ? "bg-ink text-background" : "bg-background"}>
      <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 sm:py-20">
        <div className="mb-10 max-w-2xl">
          <p className={`text-xs font-bold uppercase tracking-widest ${dark ? "text-gold" : "text-ember"}`}>{eyebrow}</p>
          <h2 className={`mt-2 font-display text-3xl font-bold sm:text-4xl lg:text-[2.65rem] ${dark ? "text-background" : "text-foreground"}`}>
            {title}
          </h2>
        </div>
        {children}
